...
colors = ["red", "blue", "grey", "orange", "black", "yellow", "red", "green", "blue", "purple", "yellow", "brown", "light_blue", "black", "white", "dark_blue", "orange"]
...
for i in range(len(shape_list)):
    if i < 5:
        ...
        y4 = shape_list[i][7]
        name = shape_list[i][8]
        rhomb = Rhombus(x1, y1, x2, y2, x3, y3, x4, y4, name)
        ...
        x5 = shape_list[i][8]
        y5 = shape_list[i][9]
        name = shape_list[i][10]
        penta = RegularPentagon(x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, name)
        finish_list.append(penta)

for i in range(len(finish_list)):
    c = colors[i]
    finish_list[i].fill(c)


for i in range(len(finish_list)):
        print("Площадь фигуры " + str(finish_list[i].name) + ": " + str(finish_list[i].square(finish_list[i].s_list)))
        print("Периметр фигуры " + str(finish_list[i].name) + ": " + str(finish_list[i].perimeter(finish_list[i].s_list)))
        print("Цвет фигуры " + str(finish_list[i].name) + ": " + str(finish_list[i].color))
        print(" ")
...
for i in range(len(finish_list)):
        while i < len(finish_list) - 1:
            finish_list[i].compare(finish_list[i], finish_list[i + 1])
            finish_list[i].is_intersect(finish_list[i], finish_list[i + 1])
            finish_list[i].is_include(finish_list[i], finish_list[i + 1])
            print()
            i += 1

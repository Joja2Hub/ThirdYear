...
def insertData(con):
    ...
    id = form.getfirst("country_id", "-default-")
    id = html.escape(id)
    temp_list = []
    temp_list.append(id)
    if id == "-default-":
        ...
        cursorObj.execute("DELETE FROM Countries WHERE country_id=" + id)
        message = f"<p>Страна с id '{id}' успешно удалена из базы данных.</p>"
